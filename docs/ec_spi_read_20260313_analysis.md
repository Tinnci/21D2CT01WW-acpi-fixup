...
基于 `ec_spi_read_20260313_104036.bin` 的结构化扫描，可分为以下功能区：

1. **EC 容器头与引导参数区**
   - `_EC2` 头：`0x001000`
   - Boot Config magic：`0x0010C0`
   - 向量表：`0x001454`
   - 版本主串：`N3GHT15W @ 0x001508`

2. **主执行固件区（早期段）**
   - `0x00002500-0x000443FF`（含大量可执行数据与版本串）
   - 关键字符串出现：`N3GPD04W`、`N3GPH03W`、`N3GPH53W`

3. **中后段扩展数据区（疑似配置/资源/压缩块）**
   - `0x01C50000-0x01CB6AFF`（夹杂零填充区）
   - `0x01D00000-0x01FFFFFF`（连续 3MB DATA）

4. **大块空洞/擦除区**
   - 多处 `00` 或 `FF` 连续区域，说明镜像是“稀疏布局”而非全片连续固件。

模块化风险判断：

- 当前样本并不符合“FL2 payload 直接平铺”模型。
- 不能用单一偏移把量产 FL2 整段覆盖到当前镜像。
- 推荐策略仍是“保留头部 + 分区映射替换 + 校验回读”。
