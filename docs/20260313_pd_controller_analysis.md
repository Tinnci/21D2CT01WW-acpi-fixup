...
## 3.14 KB8002 手动写入测试续：reg[0x27] 命令寄存器触发和恢复（2026-03-14）

### 3.14.1 测试过程

在 3.13.8 节手动写入验证计划基础上，本节记录实际执行结果。

**I2C Bus 确认**：KB8002 在 `i2c-0`（Synopsys DesignWare 0）上响应：
- port1: `0x27`（返回 `0x28`）
- port2: `0x48`（返回 `0x04`）
...
```
port1 (0x27):
  reg[0x00] = 0x28   reg[0x01] = 0x41   reg[0x02] = 0x01
  reg[0x03] = 0x41   reg[0x04] = 0x49   reg[0x05] = 0xb4
  reg[0x06] = 0x4e   reg[0x0d] = 0x98   reg[0x0f] = 0xbd
  reg[0x15] = 0x08   reg[0x16] = 0x0a   reg[0x17] = 0x08
  reg[0x19] = 0x08   reg[0x1a] = 0x0d   reg[0x27] = 0x05
  reg[0x28] = 0x02   reg[0x29] = 0x51   reg[0x2d] = 0x30
  reg[0x2e] = 0x63   reg[0x2f] = 0x54   reg[0x30] = 0x04
  reg[0x32] = 0x01   reg[0x33] = 0x03   reg[0x34] = 0xf4
  reg[0x35] = 0x45   reg[0x37] = 0x3e   reg[0x3f] = 0x0f
  reg[0x40] = 0x3c   reg[0x42] = 0x1a   reg[0x47] = 0x06
  reg[0x4a] = 0x01   reg[0x51] = 0x03   reg[0x52] = 0x03
  reg[0x55] = 0x3e   reg[0x56] = 0x3f   reg[0x5c] = 0xd2
  reg[0x5f] = 0x81   reg[0x64] = 0x0c   reg[0x69] = 0x01
  reg[0x70] = 0x05   reg[0x7e] = 0x01
  (共 41 个非零寄存器)
```

### 3.14.2 关键发现：reg[0x22] 和 reg[0x42] 写入无效

```bash
# 测试写入 reg[0x22] = 0x02 (byte 写)
sudo i2ctransfer -y 0 w3@0x27 0x22 0x00 0x02  # ACK（无错误）
sudo i2ctransfer -y 0 w2@0x27 0x22 0x00 r1    # 读回 → 0x00（写入无效）

# 测试写入 reg[0x42] = 0x0078 (word 写)
sudo i2ctransfer -y 0 w4@0x27 0x42 0x00 0x78 0x00  # ACK
sudo i2ctransfer -y 0 w2@0x27 0x42 0x00 r2          # 读回 → 0x1a 0x00（写入无效）
```

**结论**：reg[0x22] 和 reg[0x42] 均为**只读状态寄存器**，不是写入目标，而是反映当前链路协商结果的只读值。EC 写入的机制是通过 USB-C 链路协商训练流程触发 KB8002 内部更新这些寄存器。

### 3.14.3 意外触发：reg[0x27] 是命令/控制寄存器

在测试写入可写寄存器时，执行了：
```bash
sudo i2ctransfer -y 0 w3@0x27 0x27 0x00 0x07  # 写 reg[0x27]=0x07
```

**结果**：port1(0x27) 立即停止响应所有 I2C 访问（Remote I/O error），而 port2(0x48) 仍正常。

**分析**：
- `reg[0x27]` 原值 = `0x05` = `0b00000101`
- 写入值 = `0x07` = `0b00000111`（bit1 额外置位）
- 可能的含义：bit1 是系统复位命令触发位，或者 port1 进入低功耗/待机模式

**重要**：`reg[0x27]` **不是普通状态寄存器**，可能是命令寄存器（COMMAND REG），写入特定位会触发 KB8002 内部操作（复位、关闭端口等）。

### 3.14.4 恢复方法

port1(0x27) 失联后，以下方式可能恢复：
1. **系统重启**（最安全）：EC 重新上电初始化 KB8002
2. **USB-C 热插拔**：触发 EC 状态机重新运行，可能恢复 port1
3. **通过 port2 发送全局复位命令**（需要确认 KB8002 跨端口命令寄存器）
...
基于本次测试和 EC 逆向分析，KB8002 寄存器分区：

| 寄存器范围 | 类型       | 示例                                      |
|-----------|------------|-------------------------------------------|
| 0x00~0x06 | 只读标识    | 0x28=VendorID, 0x41='A', 0x49='I'=chip ID |
| 0x0d~0x1a | 只读状态    | 链路训练结果、能力反馈                     |
| 0x22       | 只读状态    | 当前 USB3/AUX 模式（EC 写的来源是链路状态）|
| 0x27       | ⚠️命令寄存器 | bit1=RESET？写入需谨慎                    |
| 0x2e~0x2f | 只读配置    | 0x63='c', 0x54='T'（可能是固件版本）       |
| 0x42       | 只读状态    | 当前 Lane 配置（0x1a = USB3 + DP 2lane?） |
| 0x70       | 只读/控制   | 初始模式码（0x05）                        |

### 3.14.6 reg[0x42]=0x1a 的含义分析

```
0x1a = 0001 1010 (当前工程机值)

KB8002 Lane 配置解读（推测）:
  bit[4:3] = 11 → DP 2 lanes（全功能DP）
  bit[1]   = 1  → USB3 超高速
  bit0     = 0  → 方向：正向
...
目标值 0x78 = 0111 1000:
  bit[6]   = 1  → 某个使能位
  bit[5]   = 1  → 某个使能位  
  bit[4:3] = 11 → DP 2 lanes
  bit[2:1] = 00 → USB3 或其他模式
  → 可能是 DP Only（4 lanes）或完全不同的 encoding

目标值 0x7a = 0111 1010:
  相比 0x78，bit1=1
```

**合理推断**：这两个目标值是 H70W 固件针对不同 USB-C 链路结果（USB3+DP vs DP-only）的配置，当前工程机链路未触发任何一种。

---

**本次测试总结**：手动写入 KB8002 核心功能寄存器（reg[0x22]/reg[0x42]）不生效，证实这些寄存器是只读的；意外发现 `reg[0x27]` 是命令寄存器，需要系统重启恢复 port1。根本问题仍是 EC 状态机未触发初始化流程。
