...
| ⚠️ PSP 区域替换 | 中 | **Root Key 一致，理论可行** | 用量产 ABL2+ABL_POST 替换工程版；密钥验证通过 → 参见 §25 |
...
~~USB4/USB-C SuperSpeed 在此工程样机上无法通过软件/固件补丁实现。~~

**关键发现：工程版和量产版 Root Public Key 完全一致**（参见 §25.1）。这意味着量产 ABL 模块的签名验证可以通过工程版 PSP，ABL 移植方案可行。继续参见 §25。

---

## §25 ABL 固件移植 — 量产 ABL2+ABL_POST 注入

**日期**: 2025-03-10
**目标**: 将量产 BIOS 的 ABL2 和 ABL_POST 模块移植到工程 SPI，使 PSP 获得 USB4 初始化能力

### 25.1 Root Public Key 验证

移植可行性的前提：PSP 使用 RSA 签名验证所有固件模块。如果签名不匹配，PSP 拒绝加载。

**验证方法**：提取两个镜像 $PL2 directory 的 PUB_KEY (type 0x00) 条目，逐字节比较。

```
ENG PUB_KEY @ 0x0C5400 (1088 bytes) — 首 16 bytes: 01000000bf7ea97f03f24c76ac7a40af
PROD PUB_KEY @ 0x0C5400 (1088 bytes) — 首 16 bytes: 01000000bf7ea97f03f24c76ac7a40af
结果: 完全一致 ✓
```

Root Key 一致意味着：**量产 ABL 模块的 $PS1 签名可以被工程 PSP 接受。**

### 25.2 源模块提取

从量产 FL1 (`N3GET74W/$0AN3G00.FL1`, skip 0x320 capsule header) 提取：

| 模块 | 类型 | 大小 | $PS1 Header |
|------|------|------|-------------|
| ABL2 | 0x0C | 143,616 bytes (0x023100) | ✓ body_size=0x022F00 |
| ABL_POST | 0x5F | 393,216 bytes (0x060000) | ✗ 不同格式 |

工具命令：
```bash
cargo run -p xtask -- extract -i PROD_FL1 -t ABL2 -o /tmp/prod_abl2.bin --skip 0x320
cargo run -p xtask -- extract -i PROD_FL1 -t ABL_POST -o /tmp/prod_abl_post.bin --skip 0x320
```

### 25.3 空间分析

工程 ABL2 紧邻 SMU_FW（零间隙），无法原地扩展：
```
ENG ABL2: 0x133700 - 0x153800 (131,328 bytes)
SMU_FW:   0x153800 - ...       (紧随其后)
需要额外: 0x3000 (12,288 bytes)
```

SPI 空闲区域扫描（32MB 中有 20MB 空闲）：

| 区域 | 大小 | 用途 |
|------|------|------|
| 0x2F0000 - 0x445000 | 1.33 MB | Primary ABL2+ABL_POST |
| 0x6B8000 - 0x80D000 | 1.33 MB | Backup ABL2+ABL_POST |

**关键设计**：使用同一 DirRelative 偏移 (+0x22B000 / +0x24F000) 同时适配 Primary 和 Backup $PL2：

```
Primary $PL2 (0x0C5000) + 0x22B000 = 0x2F0000  ← 在空闲区域 ✓
Backup  $PL2 (0x48D000) + 0x22B000 = 0x6B8000  ← 在空闲区域 ✓
Primary $PL2 (0x0C5000) + 0x24F000 = 0x314000  ← ABL_POST ✓
Backup  $PL2 (0x48D000) + 0x24F000 = 0x6DC000  ← ABL_POST ✓
```

### 25.4 spi-tool 实现

创建了 Rust workspace (`spi-tool/`) 用于 SPI 镜像分析和修改：

**库 crate** (`crates/spi-image/`):
- EFS 解析 (AA55AA55 签名, PSP/BIOS dir 指针)
- PSP 目录解析 ($PSP/$PL2/$BHD/$BL2, FwType 枚举 50+ 类型)
- 地址模式解析 (Direct/SpiFlash/DirRelative/Both)
- $PS1 固件头解析
- APCB 头/校验和解析
...
- `extract` — 提取固件条目
- `patch` — 替换现有条目
- `transplant` — ABL2+ABL_POST 跨镜像移植

### 25.5 移植操作

`xtask transplant` 自动化执行以下步骤：

1. **验证 Root Key 一致性** ✓
2. **提取量产 ABL2 (143KB) 和 ABL_POST (393KB)** ✓
3. **扫描公共空闲区域** — 找到 +0x22B000 ✓
4. **写入 blob 到 4 个位置**：
   - ABL2 → Primary 0x2F0000, Backup 0x6B8000
   - ABL_POST → Primary 0x314000, Backup 0x6DC000
5. **更新 $PL2 目录**：
   ...
   - entry_count: 43→44 (两个 $PL2 都更新)
6. **清零旧 ABL2 位置** (0xFF fill) ✓

### 25.6 APCB 叠加补丁

在移植镜像基础上叠加 APCB USB4 tokens（复用 §22 的 Primary-only 补丁方案）：

```
修改字节: 10 data + 1 checksum = 11 bytes (仅 Primary APCB @ 0x449000)
Backup APCB @ 0x811000: 未修改（安全回退网）
```
...
```bash
# 备份当前 SPI
sudo flashrom -p internal -c W25Q256JW -r build/pre_abl_transplant_dump.bin

# 写入 ABL 移植 + APCB 补丁镜像
sudo flashrom -p internal -c W25Q256JW -w build/spi_primary_only_patch_20260310_234543.bin

# 校验
sudo flashrom -p internal -c W25Q256JW -v build/spi_primary_only_patch_20260310_234543.bin
# 结果: VERIFIED ✓
```
...
| 组件 | 状态 | 来源 |
|------|------|------|
| PSP_BL, FW_REC, SMU_* | 原始工程版 | 未修改 |
| ABL0_AB | 原始工程版 | 未修改 |
| **ABL2** | **量产版 (N3GET74W)** | 143KB, DirRelative+0x22B000 |
| **ABL_POST** | **量产版 (N3GET74W)** | 393KB, DirRelative+0x24F000 |
| AGESA_BL | 原始工程版 (131KB) | 未修改（vs 量产 143KB） |
| DXIO_PHY | 原始工程版 (5KB) | 未修改（vs 量产 6KB） |
| APCB Primary | **USB4 补丁版** | 11 bytes modified |
| APCB Backup | 原始工程版 | 安全回退 |
| BIOS volumes | 原始工程版 | 未修改 |

### 25.9 预期结果与风险

**最佳情况 (~35%)**:
PSP 接受量产 ABL2 签名，ABL 读取 APCB USB4 tokens，初始化 NHI/USB4 硬件，USB-C 端口提升到 SuperSpeed。

**中等情况 (~35%)**:
系统正常启动但 USB4 无变化。原因可能：
- 工程版 AGESA_BL (131KB) 与量产 ABL2 接口不兼容
- 工程版 DXIO_PHY 缺少 USB4 PHY 配置
- ABL_POST 需要也移植 AGESA_BL 才能正常运行

**不利情况 (~20%)**:
ABL2 加载失败，PSP halt 或回退。系统可能无法启动。
- 恢复: `flashrom -w firmware/spi_dump/bios_dump_20260306_104957.bin`

**极端情况 (<10%)**:
PSP 锁定 SPI 写保护。需要外接 SPI 编程器恢复。

**恢复文件**:
- 原始工程 dump: `firmware/spi_dump/bios_dump_20260306_104957.bin`
