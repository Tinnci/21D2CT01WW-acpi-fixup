...
def analyze_dump(dump_path, fl2_paths):
    ...
    print(f"  Data bytes: {len(dump)-ff_count-zero_count} ({100*(len(dump)-ff_count-zero_count)/len(dump):.1f}%)")
    ...
    for fl2_path in fl2_paths:
        ...
        if pos >= 0:
            ...
            if end <= len(dump):
                matched = sum(1 for i in range(len(payload))
                ...
                if matched != total:
                    ...
                    first_diff = next(i for i in range(total)
                                      if dump[pos + i] != payload[i])
                    last_diff = next(i for i in range(total - 1, -1, -1)
                                     if dump[pos + i] != payload[i])
