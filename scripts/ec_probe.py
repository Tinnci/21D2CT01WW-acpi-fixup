...
def cmd_vendor_query(ec, args):
    ...
    try:
        ...
        time.sleep(0.01)
        status = ec.ec_status()
        print(f"  Status after 0xCF: 0x{status:02X} (OBF={bool(status & OBF)}, IBF={bool(status & IBF)}, SCI={bool(status & SCI_EVT)})")
        
        if status & OBF:
            val = ec.inb(EC_DATA_PORT)
            print(f"  Response: 0x{val:02X}")
        else:
            print(f"  No response data (OBF not set)")
    ...
    for start, end, desc in regions:
        ...
        for addr in range(start, end + 1):
            try:
                ...
                values.append("??")
        ascii_repr = ''.join(
            chr(int(v, 16)) if v != '??' and 32 <= int(v, 16) < 127 else '.'
            for v in values
        )
...
def cmd_probe_extended_protocol(ec, args):
    ...
    try:
        ...
        try:
            ...
            print(f"  0xA0 + 0x00 + 0x00 + 0x00: No response")
    except TimeoutError:
        print(f"  Timeout during 3-byte protocol")
    time.sleep(0.02)
    if ec.ec_status() & OBF:
        ec.inb(EC_DATA_PORT)
