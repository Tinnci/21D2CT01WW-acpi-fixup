...
def main():
    ...
    for instance_name, info in APCB_INSTANCES.items():
        ...
        if stored_size != size or stored_uid != info["uid"]:
            print(f"  ✗ APCB metadata mismatch: size=0x{stored_size:X} (exp 0x{size:X}), uid=0x{stored_uid:X} (exp 0x{info['uid']:X})")
            failures += 1
            continue
    ...
    print("NEXT STEPS")
    print(f"{'='*70}")
    print(f"  1. Connect SPI programmer (CH341A) to Winbond W25Q256JW")
    print(f"  2. Verify chip ID: flashrom -p ch341a_spi")
    print(f"  3. Read-back verify: flashrom -p ch341a_spi -r /tmp/readback.bin")
    print(f"  4. Compare: sha256sum /tmp/readback.bin  (should match original dump)")
    print(f"  5. Flash: flashrom -p ch341a_spi -w {output_path}")
    print(f"  6. Read-back verify again after flashing")
    print(f"  7. Boot and check: lspci -s 00:08 -vv  (look for USB4 NHI)")
    print(f"  8. If USB4 NHI appears: modprobe thunderbolt")
