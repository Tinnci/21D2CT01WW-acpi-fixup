...
def main():
    ...
    print("╔═══════════════════════════════════════════════════════╗")
    print("║  APCB USB4 Patch — Primary Only (方案B)              ║")
    print("║  保留 Backup APCB 作为 ABL fallback 安全网           ║")
    print("╚═══════════════════════════════════════════════════════╝")
    print()
    ...
    print(
        f"  Original checksum: 0x{data[APCB_PRIMARY_BASE + CHECKSUM_OFFSET]:02X} (block sum: 0x{orig_sum:02X} {'✓' if orig_sum == 0 else '✗ INVALID'})"
    )

    if orig_sum != 0:
        print("  ERROR: Original APCB checksum invalid! Aborting.")
        sys.exit(1)
    ...
    for apcb_off, old_val, new_val, desc in PATCHES:
        ...
        if current != old_val:
            ...
            print("    ERROR: Unexpected value! Image may not match expected BIOS version.")
            sys.exit(1)
    ...
    print("  未修改: Backup APCB, Small APCB")
    print("  ABL fallback: 如果 Primary 失败 → 使用原始 Backup")
    print(f"{'=' * 55}")
    print("\n  刷写命令:")
